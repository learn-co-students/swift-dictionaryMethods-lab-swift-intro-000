...
class DictionaryMethods {
    ...
    func remove(droid: String) -> Bool {
        var droidRemoved = false
        for (index, droidToBeFind) in starWarsDroids.enumerated() {
            if droidToBeFind == droid {
    ...
    func addHearts(){
        for (type, names) in starWarsCharacters {
            var nameContainingHeart = String()
            var namesContainingHearts = [String]()
            for name in names {
                nameContainingHeart = name.replacingOccurrences(of: "o", with: "❤️")
                namesContainingHearts.append(nameContainingHeart)
            }
            starWarsCharacters.updateValue(namesContainingHearts, forKey: type)
        }
    }
